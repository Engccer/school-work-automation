...
| 스킬 | 담당 영역 |
|---|---|
| **neis-automation** (나이스) | 업무포털 → 나이스 진입, 근무상황·출장 신청(육아시간·연가 등), 외부강의 신고·현황, 정기고사 일괄채점·문항 통계, 학기 시작 선행작업(성취기준·수행평가영역·반영비율), 동아리 이수시간·특기사항, 학교생활기록부 독서활동상황 입력, 상신함 결재 내역 추출 |
| **k-edufine-automation** (K-에듀파인) | 업무포털 → K-에듀파인 진입, 결재대기 공문 처리(과제카드·문서처리), 공문 본문·첨부 PC저장, 신규 기안 작성 → 수신자·결재경로 지정 → 결재올림 |
| **kyodetong-submit** (교데통) | 교데통(교육행정데이터통합관리) 데이터취합 접수 → 파일 첨부 → 임시저장 → 내부승인 결재 상신 → 완료 확인 |
