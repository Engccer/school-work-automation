...
- 업무포털: `https://sen.eduptl.kr/bpm_man_mn00_001.do` (진입·로그인은
  [neis-automation](../neis-automation/SKILL.md) 스킬의 1~3단계와 동일)
- K-에듀파인: `https://klef.sen.go.kr/keris_ui/main.do` (업무포털에서 SSO로 진입)

**원격(외부망) 접속**: K-에듀파인은 **EVPN 연결 없이는 HTTP 응답 자체가 없다**
(실측 2026-08-20: 미연결 시 타임아웃, 연결 시 302 응답). 자택 등 학교 외부에서는
[neis-automation](../neis-automation/SKILL.md) 스킬의 「원격(외부망) 접속 — EVPN」
절차(evpn.sen.go.kr → 웹 로그인 → AXGATE 클라이언트 로그인)로 터널을 먼저 연결할 것.
터널이 중간에 끊기면(실측: 약 12분 뒤 끊김 사례) K-에듀파인이 다시 무응답이 되므로,
원격 작업 중 응답이 사라지면 EVPN 재연결부터 확인한다.
...
업무포털 접속·인증서 로그인 핸드오프·공지 팝업 닫기는 neis-automation 스킬 1~3단계와
...
- v4.2 (2026-08-20): **원격(외부망) 접속 안내 추가.** K-에듀파인은 EVPN 없이는 HTTP 무응답임을
  실측으로 확인(미연결 시 타임아웃, EVPN 연결 시 302). EVPN 신청·접속 절차 상세는
  neis-automation 스킬의 「원격(외부망) 접속 — EVPN」 섹션에 정리(중복 서술 대신 참조).
